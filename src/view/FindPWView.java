...
public class FindPWView {
	public FindPWView() {
		...
		Find_PW = udao.findPW(userid, username, birth, phagency, userphone, answer);
		if (Find_PW != null) {
			System.out.println("["+username+"]" +"님이 찾으시는 비밀번호는 " + "["+Find_PW +"]"+"입니다!");
		} else {
			System.out.println("입력하신 정보가 맞지 않습니다. 다시 확인 해서 진행해 주세요 !");
		}
	}
